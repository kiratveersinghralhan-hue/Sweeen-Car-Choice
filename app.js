/*
  Sween Car Choice Ecommerce Demo
  - Paste Supabase + Razorpay keys below.
  - Works as a beautiful local demo even before configuration.
  - For real Razorpay payments, run server.js and set API_BASE_URL.
*/
...
const CONFIG = {
  ...
  API_BASE_URL: '', // Example: 'http://localhost:8787' or your Render/Railway backend URL
...
function initSupabase() {
  if (!validConfig(CONFIG.SUPABASE_URL, 'PASTE_SUPABASE_URL_HERE') || !validConfig(CONFIG.SUPABASE_ANON_KEY, 'PASTE_SUPABASE_ANON_KEY_HERE')) {
    return;
  }
  if (!window.supabase) {
...
async function checkout() {
  ...
  const razorpayReady = validConfig(CONFIG.RAZORPAY_KEY_ID, 'PASTE_RAZORPAY_KEY_ID_HERE') && CONFIG.API_BASE_URL;
  if (!razorpayReady) {
    await saveOrder({ customer, details, payment: { mode: 'demo', status: 'demo_success' } });
    cart = [];
    appliedCoupon = null;
    saveLocal('sween_pro_cart', cart);
    updateCartUI();
    toast('Demo order placed. Add Razorpay keys for real payment.');
    return;
  }

  if (typeof Razorpay === 'undefined') return toast('Razorpay script not loaded. Check internet/CDN.');

  try {
    els.checkoutBtn.disabled = true;
    els.checkoutBtn.textContent = 'Creating secure order...';
    const payload = {
      items: details.items.map(item => ({ id: item.product.id, qty: item.qty, price: item.product.price })),
      couponCode: appliedCoupon?.code || '',
      customer
    };
    const orderResponse = await fetch(`${CONFIG.API_BASE_URL}/api/create-razorpay-order`, {
      method: 'POST',
    ...
    const options = {
      ...
      notes: { address: customer.address, reason: customer.reason, shop: CONFIG.SHOP_NAME },
      theme: { color: '#b5121b' },
      handler: async function (response) {
        await verifyAndSavePayment(response, customer, details, orderData);
      }
    };
    ...
    console.error(error);
    toast('Checkout error. Check server/API keys.');
  } finally {
    els.checkoutBtn.disabled = false;
    els.checkoutBtn.textContent = 'Checkout with Razorpay';
  }
}

async function verifyAndSavePayment(response, customer, details, orderData) {
  try {
    ...
    await saveOrder({ customer, details, payment: { ...response, orderData, status: 'paid' } });
    cart = [];
    appliedCoupon = null;
    saveLocal('sween_pro_cart', cart);
    updateCartUI();
    closeCart();
    toast('Payment successful. Order saved.');
  } catch (error) {
    console.error(error);
    toast('Payment captured but verification failed. Check server logs.');
  }
}
...
async function saveOrder({ customer, details, payment }) {
  ...
  localOrders.unshift({ ...order, id: cryptoRandomId(), created_at: new Date().toISOString() });
  saveLocal('sween_pro_orders', localOrders);
  orders = localOrders;
  renderAdminOrders();
  await sendFormspree('New Sween Car Choice order', order);
}
